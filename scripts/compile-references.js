...
/** @type {YarnWorkspace} */
const LBXDRUGS_MONOREPO = {
    name: '@lbxdrugs/monorepo',
...
try {
    ...
    const references = getTypescriptReferences(LBXDRUGS_MONOREPO, configsFolder);
    result = configureTypeScriptCompilation(LBXDRUGS_MONOREPO, tsconfigCompilePath, references);
    rewriteRequired = rewriteRequired || result;

    // Configure the root `tsconfig.json` for code navigation using `tsserver`.
    const tsconfigNavPath = path.join(ROOT, 'tsconfig.json');
    result = configureTypeScriptNavigation(LBXDRUGS_MONOREPO, tsconfigNavPath);
    rewriteRequired = rewriteRequired || result;
...
function configureTypeScriptNavigation(targetPackage, tsconfigPath) {
    ...
    for (const packageName of LBXDRUGS_MONOREPO.workspaceDependencies) {
        ...
        if (fs.existsSync(depConfigPath) && fs.existsSync(depSrcPath)) {
            ...
            mappedFsPath = path.relative(LBXDRUGS_MONOREPO.location, path.join(depSrcPath, '*'));

        } else {
            // I don't really know what to do here, simply point to our local package root.
            originalImportPath = `${packageName}/*`;
            mappedFsPath = path.relative(LBXDRUGS_MONOREPO.location, path.join(depWorkspace.location, '*'));
        }
